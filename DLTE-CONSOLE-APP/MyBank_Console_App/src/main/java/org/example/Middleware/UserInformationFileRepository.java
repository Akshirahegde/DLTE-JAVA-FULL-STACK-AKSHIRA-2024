...
public class UserInformationFileRepository  implements UserInfoRepository {
    ...
    private void writeIntoFile(){
        try {
            readFromFile();
            FileOutputStream fileOutputStream = new FileOutputStream("Records.txt");
    ...
    @Override
    public void DepositAmountInto(final String username, Long amount) {
        readFromFile();
        Customer checkUsername=userList.stream().filter(each->each.getUserName().equals(username)).findFirst().orElse(null);
        if(checkUsername==null){
            logger.log(Level.INFO,resourceBundle.getString("user.name"));
        ...
        Double currentAmount=userList.get(index).getInitialBalance();
        ...
        userList.get(index).setInitialBalance(newBalance);
        writeIntoFile();
